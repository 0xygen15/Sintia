...
class Engine:
    ...
    def set_levels(self, lifestyle, absurd, relations, personal, adult):
        ...
        def get_data_truth(level):
            c.execute("SELECT question FROM TRUTH WHERE level=?", (level,))
            return [i[1] for i in c.fetchall()]

        def get_data_dare(level):
            c.execute("SELECT question FROM DARE WHERE level=?", (level,))
            return [i[1] for i in c.fetchall()]

        def get_data_never(level):
            c.execute("SELECT question FROM NEVER WHERE level=?", (level,))
            return [i[1] for i in c.fetchall()]
