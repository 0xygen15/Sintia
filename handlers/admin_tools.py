...
@dp.channel_post_handler(filters.IDFilter(chat_id=channel_id), content_types=types.ContentType.ANY)
async def forward_from_channel(message: Message):
    for chat_id in Users.chat_ids():
        await bot.forward_message(chat_id=chat_id[0], from_chat_id=channel_id, message_id=message.message_id,
